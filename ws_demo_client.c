...
// 用戶界面函數
void show_menu() {
    printf("\n=== WebSocket SIP Demo 客戶端 ===\n");
    printf("1. 撥打電話 (預設號碼: 0938220136)\n");
    printf("2. 撥打電話 (自定義號碼)\n");
    printf("3. 掛斷電話\n");
    printf("4. 顯示統計\n");
    printf("5. 退出\n");
    printf("6. 將接收的 RTP 封包保存為 WAV 文件\n");
    printf("請選擇 (1-6): ");
    fflush(stdout);
}
            ...
            case 6:
                {
                    ...
                    // 生成帶時間戳的文件名
                    strftime(filename, sizeof(filename), "rtp_capture_%Y%m%d_%H%M%S.wav", timeinfo);
                    
                    log_with_timestamp("正在將接收到的 RTP 數據保存為 WAV 文件: %s\n", filename);
                    if (save_rtp_to_wav(filename) == 0) {
...
int main(int argc, char **argv) {
    ...
    log_with_timestamp("WebSocket SIP Demo 客戶端啟動\n");
    ...
    connect_info.protocol = "sip-demo-protocol";
    ...
    lws_context_destroy(context);
    free_rtp_buffer();
    log_with_timestamp("WebSocket 客戶端已關閉\n");
